...
import ru.spliterash.minestomEvent.core.trait.RecursiveEvent;
import ru.spliterash.minestomEvent.core.utils.Check;
...
class EventNodeImpl<T extends Event> implements EventNode<T> {
    ...
    @Override
    public @NotNull EventNode<T> addListener(@NotNull ru.spliterash.minestomEvent.core.EventListener<? extends T> listener) {
        synchronized (GLOBAL_CHILD_LOCK) {
            final var eventType = listener.eventType();
            ListenerEntry<T> entry = getEntry(eventType);
            entry.listeners.add((ru.spliterash.minestomEvent.core.EventListener<T>) listener);
    ...
    @Override
    public @NotNull EventNode<T> removeListener(@NotNull ru.spliterash.minestomEvent.core.EventListener<? extends T> listener) {
        synchronized (GLOBAL_CHILD_LOCK) {
            final var eventType = listener.eventType();
    ...
    Graph createGraph() {
        synchronized (GLOBAL_CHILD_LOCK) {
            List<Graph> children = this.children.stream().map(EventNodeImpl::createGraph).collect(Collectors.toUnmodifiableList());
    ...
    static final class Graph {
        ...
        public Graph(String name, String eventType, int priority, List<Graph> children) {
            ...
            this.priority = priority;
            this.children = children.stream().sorted(Comparator.comparingInt(Graph::priority)).collect(Collectors.toUnmodifiableList());;
        }
    }
    ...
    private static class ListenerEntry<T extends Event> {
        final List<ru.spliterash.minestomEvent.core.EventListener<T>> listeners = new CopyOnWriteArrayList<>();
    ...
    final class Handle<E extends Event> implements ListenerHandle<E> {
        ...
        /**
         * Create a consumer calling all listeners from {@link EventNode#addListener(ru.spliterash.minestomEvent.core.EventListener)} and
        ...
        private @Nullable Consumer<E> listenersConsumer(@NotNull ListenerEntry<E> entry) {
            final ru.spliterash.minestomEvent.core.EventListener<E>[] listenersCopy = entry.listeners.toArray(ru.spliterash.minestomEvent.core.EventListener[]::new);
            ...
            if (bindingsEmpty && listenersCopy.length == 1) {
                ...
                final ru.spliterash.minestomEvent.core.EventListener<E> listener = listenersCopy[0];
                return e -> callListener(listener, e);
            }
            // Worse case scenario, try to run everything
            return e -> {
                if (!listenersEmpty) {
                    for (ru.spliterash.minestomEvent.core.EventListener<E> listener : listenersCopy) {
        ...
        void callListener(@NotNull ru.spliterash.minestomEvent.core.EventListener<E> listener, E event) {
            var node = (EventNodeImpl<E>) EventNodeImpl.this;
            ru.spliterash.minestomEvent.core.EventListener.Result result = listener.run(event);
